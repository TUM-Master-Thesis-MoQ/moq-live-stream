...
keywords=("Current frame delay rate")
...
for keyword in "${keywords[@]}"; do
    ...
    echo "Chunk Index,Delay Rate" > "$output_csv"
    ...
    while IFS= read -r line; do
        ...
        delayRate=$(echo "$line" | grep -oE "$keyword: [0-9]+(\.[0-9]+)?" | sed "s/$keyword: //")
        
        # If value found, append it to the CSV
        if [[ -n $delayRate ]]; then
            echo "$index,$delayRate" >> "$output_csv"
            index=$((index + 1))
        fi
    done < "$input_file"
    ...
    python3 ../plotter.py "Chunk Index" "Delay Rate" "$output_csv"
done