...
for keyword in "${keywords[@]}"; do
    ...
    echo "Chunk Index,Video Buffer Size" > "$output_csv"
    ...
    while IFS= read -r line; do
        ...
        videoBufferSize=$(echo "$line" | grep -oE "$keyword: [0-9]+" | sed "s/$keyword: //")
        
        # If value found, append it to the CSV
        if [[ -n $videoBufferSize ]]; then
            echo "$index,$videoBufferSize" >> "$output_csv"
            index=$((index + 1))
        fi
    done < "$input_file"
    ...
    python3 ../plotter.py "Chunk Index" "Video Buffer Size" "$output_csv"
done